...
def hash_160_to_bc_address(h160, addrtype = 15):
...
def SecretToASecret(secret, compressed=False, addrtype=15):
    vchIn = chr((addrtype+128)&255) + secret
    if compressed: vchIn += '\01'
    return EncodeBase58Check(vchIn)

def ASecretToSecret(key, addrtype=15):
...
def msg_magic(message):
    ...
    return "\x18MonetaryUnit Signed Message:\n" + encoded_varint + message
