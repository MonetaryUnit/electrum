...
def user_dir():
    if "HOME" in os.environ:
        return os.path.join(os.environ["HOME"], ".mue-electrum")
    elif "APPDATA" in os.environ:
        return os.path.join(os.environ["APPDATA"], "MUE-Electrum")
    elif "LOCALAPPDATA" in os.environ:
        return os.path.join(os.environ["LOCALAPPDATA"], "MUE-Electrum")
    elif 'ANDROID_DATA' in os.environ:
        return "/sdcard/mue-electrum/"
    else:
        #raise Exception("No home directory found in environment variables.")
        return

def format_satoshis_plain(x, decimal_point = 5):
    '''Display a satoshi amount scaled.  Always uses a '.' as a decimal
    point and has no thousands separator'''
    scale_factor = pow(10, decimal_point)
    return "{:.8f}".format(Decimal(x) / scale_factor).rstrip('0').rstrip('.')

def format_satoshis(x, is_diff=False, num_zeros = 0, decimal_point = 5, whitespaces=False):
...
block_explorer_info = {
    'MUE Insight': ('https://chain.muewallet.com',
                        {'tx': 'tx', 'addr': 'address'}),
}

def block_explorer(config):
    return config.get('block_explorer', 'MUE Insight')
...
def create_URI(addr, amount, message):
    ...
    p = urlparse.ParseResult(scheme='monetaryunit', netloc='', path=addr, params='', query='&'.join(query), fragment='')
